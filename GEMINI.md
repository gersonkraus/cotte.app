...
- Use **CLAUDE.md** quando estiver trabalhando com o Anthropic SDK (`ia_service.py`)
- Use **GEMINI.md** quando estiver trabalhando com Gemini (ou em testes paralelos)
- Os dois arquivos são praticamente iguais — só muda o título e a linha da IA.

Se quiser que eu faça mais alguma adaptação (ex: adicionar referência específica ao Gemini SDK ou deixar os dois arquivos idênticos), é só falar.

Quer que eu atualize algum JS agora para usar o `UXImprovements` ou `ApiService` em uma tela específica?