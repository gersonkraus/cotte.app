...
- Na geração de portfólio (PDF via WeasyPrint e visualização HTML), a exibição deve ser idêntica (sem margens extras no `@page` do PDF) e suportar layouts configuráveis (como 'compacto' e 'galeria'). Se o operador pedir, providenciar campos para omitir categorias no documento final.
- No desenvolvimento local de imagens (upload e exibição no catálogo), o `R2Service` possui fallback para gravar e servir arquivos no diretório `static/images/empresas/...` via FastAPI, evitando falhas quando o Cloudflare R2 não estiver configurado.
- Na UI do catálogo (`catalogo.html`), usar CSS para limitar dinamicamente a exibição de descrições muito grandes (ex.: `-webkit-line-clamp: 3` com tooltip via atributo `title`) a fim de não quebrar as linhas ou deformar os cards dos produtos.
