...
- Relatórios de vendas e contas a receber no assistente expõem tools dedicadas `gerar_relatorio_vendas` e `gerar_relatorio_contas_a_receber` em `financeiro_tools`, registradas no registry em `ai_tools/__init__.py` (alternativa mais estável a depender só de SQL ad-hoc).
