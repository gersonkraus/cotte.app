...
- Na tool `listar_orcamentos`, perguntas sobre aprovação em dias civis (“ontem”, “hoje”, intervalo) devem usar `aprovado_em_de`/`aprovado_em_ate` (YYYY-MM-DD, dia civil em America/Sao_Paulo); o parâmetro `dias` filtra pela data de criação (`criado_em`). O executor normaliza argumentos comuns (ajusta `dias`/`limit` e converte literais “ontem”/“hoje” para datas ISO).
...
- No assistente V2 (hub), o payload pode incluir `engine` (ex.: `operational` por omissão); o copiloto técnico interno deve usar o endpoint dedicado `POST /ai/copiloto-interno` — não enviar `engine=internal_copilot` no fluxo operacional de `/ai/assistente` (a API rejeita com 400). No registry de engines, a disponibilidade da tool `executar_sql_analitico` no engine `internal_copilot` segue a flag de SQL Agent (`is_sql_agent_enabled`), não a de Code RAG.
- `GET /ai/assistente/capabilities` expõe para o frontend flags, engines, `components` e `available_engines` (consumo típico via `CapabilityFlagsService` em `cotte-frontend`).
- No card de lista de orçamentos do assistente, o botão “Carregar mais” repassa `cursor`, `limit` e, quando o filtro for por aprovação, `aprovado_em_de`/`aprovado_em_ate` (não apenas `dias`), para manter o mesmo critério na página seguinte.
...
- No assistente IA V2, o valor de `input_tokens` reportado na resposta agrega o consumo de múltiplas chamadas LLM no mesmo turno (loop de tools e eventuais retries), não apenas uma chamada única.
