...
- **Git — artefatos locais ignorados:** O `.gitignore` inclui `.omx/`, `memory/.last-session`, `memory/.session-log`, `/*.patch`, `*.orig` e `*.rej`; não tratar como código do produto ao montar commits (salvo decisão explícita do time).
- **Orçamento público — template moderno:** Com `template_publico === 'moderno'`, `template-moderno.js` deve injetar a seção com os mesmos IDs do template clássico (`#secao-agendamento`, opções, erros) e chamar `exibirSecaoAgendamento(orc)` após renderizar; sem isso a escolha de horários não aparece para o cliente.
- **MCP Stacklit — diagnóstico:** Erro `spawn stacklit ENOENT` indica que o binário `stacklit` não está instalado ou não está no `PATH` do processo do Cursor (ex.: WSL); instalar a CLI ou ajustar o PATH, ou desativar o servidor MCP se não for usado.
