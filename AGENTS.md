...
- No assistente, relatórios de vendas e contas a receber usam as tools `gerar_relatorio_vendas` e `gerar_relatorio_contas_a_receber` em `financeiro_tools` (registry em `ai_tools/__init__.py`). O fluxo `POST /api/v1/ai/orcamento/confirmar` reutiliza o resolvedor de catálogo da tool `criar_orcamento` (vínculo a `Servico` por nome aproximado; `cadastrar_materiais_novos` controla cadastro de materiais novos).
...
- Integração NF-e: **Notaas** (NF-e/NFC-e) — após emissão, o polling de status deve usar `GET /nfe/invoices/{invoiceId}/status` (`GET /invoices/{invoiceId}/status` é de NFS-e — path errado em NF-e retorna 404 e pode aparecer como "Invoice não encontrado na Notaas"). Rejeições SEFAZ frequentes: cStat 209 (IE do emitente) — alinhar IE/CNPJ/UF nas configurações fiscais da empresa; cStat 972 (responsável técnico / `infRespTec`) — em geral cadastro no painel do projeto Notaas da API key, não só nos dados do orçamento. **Focus NFe** — `404` com `nao_encontrado` ao cadastrar empresa/certificado pode ser host errado (API de empresas só em `api.focusnfe.com.br`) ou path incorreto; `422` com `Município inválido: -` costuma ser endereço fiscal incompleto ou placeholder em cidade/UF/CEP; `codigo_municipio` IBGE (7–8 dígitos) ajuda quando disponível.
