...
- WhatsApp (Evolution): ao conectar WhatsApp próprio da empresa, o backend sugere `POST {APP_URL}/api/v1/whatsapp/webhook?instance=<nome_instancia>` (padrão `empresa-<id>` quando não há instância salva, ver `empresa.py`). Webhook comercial (instância de funil/leads): `POST {APP_URL}/api/v1/whatsapp/webhook-comercial`, esperando eventos `messages.upsert` ou `MESSAGES_UPSERT`; com `EVOLUTION_API_KEY` definida, o comercial valida `apikey` em header ou query. Em suporte, conferir na Evolution a URL HTTPS exata e os eventos; URLs sem `/api/v1` no domínio público podem depender de proxy/rewrite em relação a `APP_URL`. A Evolution API v2.x pode concatenar `/messages-upsert` ao query param `instance` ou ao valor de `apikey` na URL; o backend pode normalizar esses sufixos. Respostas **401** no webhook indicam `EVOLUTION_API_KEY` no servidor (ex.: Railway) diferente do token que a Evolution envia (header `apikey`/`x-api-key`, query `apikey`, Bearer ou corpo JSON conforme implementação em `whatsapp.py`); alinhar a mesma chave nos dois lados.
- Resposta automática genérica ao cliente no WhatsApp (ex.: mensagem de erro imediata) depois de a inbound ter sido persistida pode indicar exceção no fluxo `assistente_v2` em `cotte_ai_hub.py` (regressões com símbolos `_V2_*` ou helpers ausentes); `pytest tests/test_assistente_unificado_v2.py` ajuda a captar esse tipo de quebra antes do deploy.
