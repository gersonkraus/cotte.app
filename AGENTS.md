...
- Na geração de **portfólio** no catálogo, manter clareza para o operador sobre exibir ou omitir valores de **venda**: controlo dedicado (`exibir_preco_venda` no payload/schema, default ligado para compatibilidade) para cenários com fábrica/distribuidor (catálogo “sem valores” no PDF/HTML público quando desligado).
...
- Portfólio no catálogo: `PortfolioGenerateRequest` pode levar `servicos_ids` (curadoria item a item, ordem preservada quando suportado pelo `_build_portfolio_dict`); `GET /api/v1/catalogo/portfolio/produtos` lista produtos ativos para a grade no modal. Eventos fiscais do ciclo da NF ligados ao orçamento podem ser gravados na timeline como `HistoricoEdicao` com `tipo="nota_fiscal"`; no webhook Focus, registar entrada só quando o status efetivamente muda ajuda a evitar duplicação face ao polling/sync no serviço.
