...
- Integração NF-e: **Notaas** (legado NF-e/NFC-e) — polling de status em NF-e deve usar `GET /nfe/invoices/{invoiceId}/status` (o path `GET /invoices/{invoiceId}/status` é de NFS-e; path errado em NF-e retorna 404). Rejeições SEFAZ frequentes: cStat 209 (IE do emitente); cStat 972 (`infRespTec`) — em geral cadastro no painel do provedor da API key. **Focus NFe** — autenticação HTTP Basic com `FOCUS_TOKEN` (utilizador = token, senha vazia) e `FOCUS_AMBIENTE` coerente com o host de notas; o painel pode mostrar “token principal” e tokens por empresa (produção/homologação), mas o COTTE modela um único `FOCUS_TOKEN` no servidor. Cadastro de empresa/certificado na doc costuma usar sempre `https://api.focusnfe.com.br` — com `FOCUS_AMBIENTE=homologacao`, evitar montar esse fluxo no host `homologacao.focusnfe.com.br` (notas vs empresas); `404` com `nao_encontrado` costuma ser host/path errado ou atualização com identificador de empresa incorreto na URL; `401` sugere token ou ambiente incoerente; `422` com `Município inválido: -` costuma ser endereço fiscal incompleto; `codigo_municipio` IBGE (7–8 dígitos) ajuda.
