...
## Corpus Check
- 666 files · ~1,214,252 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 7717 nodes · 29189 edges · 460 communities detected
- Extraction: 40% EXTRACTED · 60% INFERRED · 0% AMBIGUOUS · INFERRED: 17556 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## God Nodes (most connected - your core abstractions)
1. `Usuario` - 724 edges
2. `StatusOrcamento` - 559 edges
3. `Orcamento` - 552 edges
4. `Empresa` - 550 edges
5. `Cliente` - 359 edges
6. `ModoAgendamentoOrcamento` - 282 edges
7. `LeadScore` - 261 edges
8. `StatusPipeline` - 258 edges
9. `TipoInteracao` - 255 edges
10. `CanalInteracao` - 255 edges
...
Cohesion: 0.05
Nodes (291): Converte string para enum, retornando CONVERSACAO se inválido, _build_redis_client(), cached(), CacheManager, generate_cache_key(), get_cached_config(), invalidate_cache_for_model(), Sistema de cache para repositórios e serviços. Implementa cache com Redis (fallb (+283 more)

### Community 4 - "Community 4"
Cohesion: 0.01
Nodes (240): get_admin_config(), _migrar_json_se_necessario(), Na primeira execução, importa dados do JSON legado para o banco., criar_agendamento_automatico(), _gerar_opcoes_automaticas(), liberar_pre_agendamento_lote(), listar_pre_agendamento_fila(), processar_agendamento_apos_aprovacao() (+232 more)

### Community 5 - "Community 5"
Cohesion: 0.01
Nodes (76): Context7, Command, Context7Agent, formatLibraryAsText(), getTrustScoreLabel(), detectVendorSpecificAgents(), getSelectedIdes(), getTargetDirFromSelection() (+68 more)

### Community 6 - "Community 6"
Cohesion: 0.02
Nodes (97): Cliente síncrono para apps ASGI usando httpx>=0.28.      Motivação:     - `fasta, SyncASGIClient, BaseHTTPMiddleware, BaseSettings, _compute_version(), Config, get_pricing_public(), Usa o hash curto do último commit git como versão. Fallback: 'dev'. (+89 more)

### Community 7 - "Community 7"
Cohesion: 0.07
Nodes (146): AgendamentoCalendario, AgendamentoComOpcoes, AgendamentoCreate, AgendamentoCreateComOpcoes, AgendamentoDashboard, AgendamentoOpcaoCreate, AgendamentoOpcaoOut, AgendamentoOut (+138 more)

### Community 8 - "Community 8"
Cohesion: 0.03
Nodes (128): abrirDetalhe(), abrirModalEnviarProposta(), abrirModalLead(), add_observacao(), adicionarObservacao(), alterarScore(), alterarStatusLead(), analisar_importacao() (+120 more)
...
Cohesion: 0.04
Nodes (100): abrirConfirmacaoReenvioProposta(), abrirContatosImportados(), abrirDetalhe(), abrirImportacaoLeads(), abrirModalCampanha(), abrirModalEmail(), abrirModalLead(), abrirModalLembrete() (+92 more)

### Community 12 - "Community 12"
Cohesion: 0.03
Nodes (40): _agora_ts(), IaInterpretarRateLimiter, _now(), PublicEndpointRateLimiter, RateLimitResult, Rate limit para endpoints públicos sem autenticação (aceitar/recusar/ajuste)., Rate limit para o webhook WhatsApp (POST /whatsapp/webhook).     Limite mais gen, Rate limit para recuperação de senha.     Prioriza Redis (quando configurado) e (+32 more)
...
Cohesion: 0.04
Nodes (60): abrirPreviewTemplatePublico(), atualizarBtnTema(), atualizarPreview(), atualizarPreviewForma(), atualizarPreviewNumero(), atualizarVisualizacaoTema(), _buildMockOrcamentoPublico(), _buildStaticPreviewExtras() (+52 more)
...
Cohesion: 0.04
Nodes (20): ABC, Interface abstrata para providers de WhatsApp.  Qualquer provider (Z-API, Evolut, Executa uma coroutine com retry em caso de falhas transitárias de rede.      Ten, Contrato que todo provider de WhatsApp deve cumprir., _retry_async(), WhatsAppProvider, Provider Evolution API para WhatsApp.  Documentação: https://doc.evolution-api.c, GET /instance/connect/{instance}  — retorna QR Code para parear. (+12 more)
...
Cohesion: 0.13
Nodes (25): abrirDetalhe(), abrirModalEnviarProposta(), abrirModalLead(), adicionarObservacao(), alterarScore(), alterarStatusLead(), arquivarLead(), atualizarBannerFollowUpLeads() (+17 more)
...
Cohesion: 0.15
Nodes (26): addDownloadButtons(), addMessage(), bootstrapCapabilities(), buildTechnicalFallbackReply(), downloadCSV(), downloadJSON(), ensureSessionId(), escapeHtml() (+18 more)
...
Cohesion: 0.13
Nodes (16): analyze_schema_drift(), check_critical_schema_drift(), compare_schema_drift_snapshots(), generate_auto_fix_preview(), get_schema_drift_snapshot(), list_schema_drift_snapshots(), Persiste um snapshot da análise de drift para histórico operacional., Lista snapshots mais recentes (paginação simples). (+8 more)
...
Cohesion: 0.26
Nodes (15): desconectar(), enviar_lembrete_cliente(), enviar_mensagem_texto(), enviar_orcamento_completo(), enviar_pdf(), get_provider(), get_provider_para_empresa(), get_qrcode() (+7 more)
...
Cohesion: 0.16
Nodes (12): add_observacao(), concluir_lembrete(), create_lembrete(), _lead(), list_interactions(), list_lembretes(), list_templates(), preview_template() (+4 more)
...
Cohesion: 0.19
Nodes (9): build_prompt_context(), is_enabled(), Retriever RAG por tenant integrado ao assistente., refresh_empresa_if_needed(), _should_refresh(), Store vetorial incremental (fallback lexical quando embeddings indisponíveis)., Armazém por tenant.      Implementação atual:     - mantém docs em memória por e, TenantVectorStore (+1 more)
...
Cohesion: 0.31
Nodes (12): build_engine_guardrails(), EnginePolicy, _env_flag(), get_engine_policy(), is_code_rag_enabled(), is_engine_available_for_user(), is_internal_copilot_enabled(), is_sql_agent_enabled() (+4 more)
...
Cohesion: 0.3
Nodes (12): _anexar_documento_orcamento(), _aprovar_orcamento(), _build_orcamento_response(), _duplicar_orcamento(), _editar_item_orcamento(), _editar_orcamento(), _enviar_orcamento_email(), _enviar_orcamento_whatsapp() (+4 more)
...
Cohesion: 0.54
Nodes (7): build_ai_health_summary(), build_token_chart_data(), _engine_health(), _extract_engine_from_log(), _p95(), _percent(), _safe_int()

### Community 127 - "Community 127"
Cohesion: 0.46
Nodes (7): _associar_usuarios_a_papeis(), _garantir_modulo_comercial_planos_existentes(), _gerar_permissoes_gestor(), seed_modulos_e_planos_padrao(), _upsert_modulos(), _upsert_papeis_empresa(), _upsert_planos()
...
Cohesion: 0.29
Nodes (7): get_schema_context_for_llm(), get_table_hints(), Contexto de schema do banco para o LLM SQL Planner., Retorna contexto de schema para injeção no prompt do LLM., Mapeamento de termos de negócio para tabelas., Resolve um termo de negócio para o nome da tabela., resolve_table_name()
...
Cohesion: 0.33
Nodes (4): _apply_tenant_scope(), _autofill_tenant_on_create(), Aplica filtro automático de tenant para entidades marcadas., Preenche `empresa_id` automaticamente em novas entidades tenant-scoped.
...
Cohesion: 0.6
Nodes (4): llm_sql_planner_enabled(), LLMSqlPlan, Planejador opcional de SQL via LLM para modo híbrido., try_generate_sql_from_llm()
...
## Knowledge Gaps
- **499 isolated node(s):** `LiveArtifact`, `SessionWorkingMemory`, `URL da imagem do serviço vinculado (catálogo), para exibir no orçamento.`, `Infraestrutura base para entidades tenant-scoped.`, `Marca models cujo escopo principal é `empresa_id`.` (+494 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 336`** (2 nodes): `cotte_ai_hub_patch.py`, `_v2_selected_tool_names_for_message()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 337`** (2 nodes): `test.js`, `escapeHtml()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 338`** (2 nodes): `compress_test.py`, `to_llm_payload()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 339`** (2 nodes): `test_payload.js`, `_politicaAgendamentoSelectParaPayload()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 340`** (2 nodes): `fix_fn.py`, `_v2_build_listar_orcamentos_fastpath_response()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 341`** (2 nodes): `tsup.config.ts`, `esbuildOptions()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 342`** (2 nodes): `auth-commands.test.ts`, `runCommand()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 343`** (2 nodes): `setup.test.ts`, `appendRule()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 344`** (2 nodes): `selectOrInput.ts`, `reorderOptions()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 345`** (2 nodes): `parse-input.ts`, `parseSkillInput()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 346`** (2 nodes): `tracking.ts`, `trackEvent()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 347`** (2 nodes): `check_status_envio.py`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 348`** (2 nodes): `test_tool_executor_engine_policy.py`, `test_execute_blocks_tool_not_allowed_by_engine()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 349`** (2 nodes): `check_commercial_data.py`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 350`** (2 nodes): `test_orcamento_core_service.py`, `test_criar_orcamento_core_aceita_item_com_valor_zero()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 351`** (2 nodes): `check_db.py`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 352`** (2 nodes): `test_financeiro_tools_contas_bug.py`, `test_gerar_relatorio_contas_a_receber()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 353`** (2 nodes): `test_render_pdf.py`, `test_render()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 354`** (2 nodes): `assistente-ia-payloads.js`, `buildConfirmarOrcamentoPayload()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 355`** (2 nodes): `template-moderno.js`, `renderizarTemplateModerno()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 356`** (2 nodes): `agendamentos-modal.spec.js`, `loginComToken()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 357`** (2 nodes): `configuracoes.spec.js`, `loginComToken()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 358`** (2 nodes): `financeiro.spec.js`, `loginComToken()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 359`** (2 nodes): `orcamentos.spec.js`, `loginComToken()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 360`** (2 nodes): `path-validation.ts`, `isPathWithinAllowedDirectories()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 361`** (2 nodes): `directory-tree.test.ts`, `buildTreeForTesting()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 362`** (2 nodes): `startup-validation.test.ts`, `spawnServer()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 363`** (2 nodes): `simple.ts`, `registerSimplePrompt()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 364`** (2 nodes): `resource.ts`, `registerEmbeddedResourcePrompt()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 365`** (2 nodes): `args.ts`, `registerArgumentsPrompt()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 366`** (2 nodes): `completions.ts`, `registerPromptWithCompletions()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 367`** (2 nodes): `prompts.test.ts`, `createMockServer()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 368`** (2 nodes): `registrations.test.ts`, `createMockServer()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 369`** (2 nodes): `tools.test.ts`, `createMockServer()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 370`** (2 nodes): `stdio.ts`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 371`** (2 nodes): `roots.ts`, `syncRoots()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 372`** (2 nodes): `trigger-sampling-request.ts`, `registerTriggerSamplingRequestTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 373`** (2 nodes): `get-tiny-image.ts`, `registerGetTinyImageTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 374`** (2 nodes): `trigger-long-running-operation.ts`, `registerTriggerLongRunningOperationTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 375`** (2 nodes): `toggle-simulated-logging.ts`, `registerToggleSimulatedLoggingTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 376`** (2 nodes): `trigger-elicitation-request.ts`, `registerTriggerElicitationRequestTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 377`** (2 nodes): `get-structured-content.ts`, `registerGetStructuredContentTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 378`** (2 nodes): `echo.ts`, `registerEchoTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 379`** (2 nodes): `trigger-elicitation-request-async.ts`, `registerTriggerElicitationRequestAsyncTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 380`** (2 nodes): `trigger-sampling-request-async.ts`, `registerTriggerSamplingRequestAsyncTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 381`** (2 nodes): `get-roots-list.ts`, `registerGetRootsListTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 382`** (2 nodes): `get-env.ts`, `registerGetEnvTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 383`** (2 nodes): `get-resource-reference.ts`, `registerGetResourceReferenceTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 384`** (2 nodes): `get-resource-links.ts`, `registerGetResourceLinksTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 385`** (2 nodes): `toggle-subscriber-updates.ts`, `registerToggleSubscriberUpdatesTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 386`** (2 nodes): `get-annotated-message.ts`, `registerGetAnnotatedMessageTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 387`** (2 nodes): `get-sum.ts`, `registerGetSumTool()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 388`** (2 nodes): `explain_performance_orcamentos_sidebar_catalogo.py`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 389`** (1 nodes): `patch_comercial.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 390`** (1 nodes): `patch_executor.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 391`** (1 nodes): `playwright.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 392`** (1 nodes): `rewrite_render.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 393`** (1 nodes): `patch_cotte_ai_hub.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 394`** (1 nodes): `patch.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 395`** (1 nodes): `patch_recusar.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 396`** (1 nodes): `patch_render_types.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 397`** (1 nodes): `patch_logging.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 398`** (1 nodes): `system.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 399`** (1 nodes): `constants.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 400`** (1 nodes): `logger.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 401`** (1 nodes): `query.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 402`** (1 nodes): `Extrai JSON válido de texto da IA.                  Args:             text: Text`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 403`** (1 nodes): `Tenta uma estratégia específica de extração`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 404`** (1 nodes): `Extrai JSON de codeblocks markdown ```json ... ````
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 405`** (1 nodes): `Extrai JSON usando regex com balanceamento de chaves.         Encontra o primeir`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 406`** (1 nodes): `Extrai JSON localizando o primeiro '{' e o último '}'.         Útil quando a IA`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 407`** (1 nodes): `Estratégia greedy - pega tudo entre o primeiro { e último }`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 408`** (1 nodes): `Extrai JSON e retorna metadados sobre o processo.                  Returns:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 409`** (1 nodes): `Retorna o status de conexão da instância (deve incluir chave 'connected': bool).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 410`** (1 nodes): `Retorna o QR Code para conectar o WhatsApp (chave 'qrcode' em base64).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 411`** (1 nodes): `Desconecta a instância. Retorna True em caso de sucesso.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 412`** (1 nodes): `Envia texto simples. Retorna True em caso de sucesso.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 413`** (1 nodes): `Envia um arquivo PDF como anexo. Retorna True em caso de sucesso.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 414`** (1 nodes): `Envia o orçamento ao cliente (link clicável + PDF).         O dict 'orcamento' c`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 415`** (1 nodes): `Notifica o operador quando o cliente abre o orçamento pela primeira vez.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 416`** (1 nodes): `Notifica o operador quando o cliente aceita o orçamento.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 417`** (1 nodes): `Notifica o operador quando o cliente recusa o orçamento.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 418`** (1 nodes): `Envia lembrete automático ao cliente sobre orçamento pendente.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 419`** (1 nodes): `Garante DDI 55 nos dígitos: 5548999887766`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 420`** (1 nodes): `Calcula a data de validade a partir de hoje + dias, retorna no formato DD/MM/AAA`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 421`** (1 nodes): `Assets do frontend não devem consumir o contador: um carregamento de página`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 422`** (1 nodes): `direct_test.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 423`** (1 nodes): `test_write.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 424`** (1 nodes): `simple_test.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 425`** (1 nodes): `debug_test.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 426`** (1 nodes): `sw.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 427`** (1 nodes): `generate_icon.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 428`** (1 nodes): `tenant-comercial-onboarding.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 429`** (1 nodes): `api-financeiro.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 430`** (1 nodes): `modal-orcamento.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 431`** (1 nodes): `assistente-render-types-fallback.test.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 432`** (1 nodes): `assistente-render-pending-action.test.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 433`** (1 nodes): `assistente-intents.test.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 434`** (1 nodes): `assistente-ia-html-fallback.test.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 435`** (1 nodes): `assistente-orcamento-confirmar-payload.test.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 436`** (1 nodes): `test-ai-response.spec.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 437`** (1 nodes): `assistente-ia-desktop.spec.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 438`** (1 nodes): `assistente-ia-mobile.spec.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 439`** (1 nodes): `assistente-ia-embed.spec.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 440`** (1 nodes): `structured-content.test.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 441`** (1 nodes): `lib.test.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 442`** (1 nodes): `roots-utils.test.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 443`** (1 nodes): `path-utils.test.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 444`** (1 nodes): `file-path.test.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 445`** (1 nodes): `knowledge-graph.test.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 446`** (1 nodes): `Test that fetching is allowed when robots.txt returns 404.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 447`** (1 nodes): `Test that fetching is blocked when robots.txt returns 401.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 448`** (1 nodes): `Test that fetching is blocked when robots.txt returns 403.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 449`** (1 nodes): `Test that fetching is allowed when robots.txt allows all.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 450`** (1 nodes): `Test that fetching is blocked when robots.txt disallows all.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 451`** (1 nodes): `Test fetching an HTML page returns markdown content.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 452`** (1 nodes): `Test fetching an HTML page with raw=True returns original HTML.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 453`** (1 nodes): `Test fetching JSON content returns raw content.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 454`** (1 nodes): `Test that 404 response raises McpError.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 455`** (1 nodes): `Test that 500 response raises McpError.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 456`** (1 nodes): `Test that proxy URL is passed to client.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 457`** (1 nodes): `resources.test.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 458`** (1 nodes): `server.test.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 459`** (1 nodes): `sse.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Usuario` connect `Community 0` to `Community 1`, `Community 34`, `Community 3`, `Community 4`, `Community 7`, `Community 8`, `Community 114`, `Community 179`, `Community 116`, `Community 60`?**
  _High betweenness centrality (0.063) - this node is a cross-community bridge._
- **Why does `Empresa` connect `Community 0` to `Community 1`, `Community 3`, `Community 6`, `Community 7`, `Community 8`, `Community 44`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **Why does `StatusOrcamento` connect `Community 0` to `Community 1`, `Community 3`, `Community 12`, `Community 141`, `Community 145`, `Community 155`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Are the 722 inferred relationships involving `Usuario` (e.g. with `EmpresaRepository` and `Repositório para operações com empresas.`) actually correct?**
  _`Usuario` has 722 INFERRED edges - model-reasoned connections that need verification._
- **Are the 557 inferred relationships involving `StatusOrcamento` (e.g. with `OrcamentoRepository` and `Repositório especializado para orçamentos.`) actually correct?**
  _`StatusOrcamento` has 557 INFERRED edges - model-reasoned connections that need verification._
- **Are the 549 inferred relationships involving `Orcamento` (e.g. with `OrcamentoRepository` and `Repositório especializado para orçamentos.`) actually correct?**
  _`Orcamento` has 549 INFERRED edges - model-reasoned connections that need verification._
- **Are the 548 inferred relationships involving `Empresa` (e.g. with `Monta a lista de origens CORS a partir do .env.` and `Garante que erros 500 retornem JSON; não sobrescreve HTTPException (400, 401, 40`) actually correct?**
  _`Empresa` has 548 INFERRED edges - model-reasoned connections that need verification._