...
def test_validate_analytics_sql_permite_union():
    """UNION é PERMITIDO na v2 (SELECT livre)."""
    result = validate_analytics_sql(
        "SELECT id FROM clientes WHERE empresa_id = :empresa_id UNION SELECT id FROM orcamentos WHERE empresa_id = :empresa_id"
    )
    assert result.ok is True


def test_validate_analytics_sql_bloqueia_union_sem_tenant_scope():
    """UNION sem tenant scope é bloqueado."""
    result = validate_analytics_sql(
        "SELECT id FROM clientes UNION SELECT id FROM orcamentos"
    )
    assert result.ok is False
    assert result.code == "sql_missing_tenant_scope"
