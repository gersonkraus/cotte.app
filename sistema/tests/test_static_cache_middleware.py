...
@pytest.fixture
def static_cache_client() -> SyncASGIClient:
    app = FastAPI()
...
def test_app_js_gets_long_cache(static_cache_client: SyncASGIClient) -> None:
...
def test_app_html_gets_no_cache(static_cache_client: SyncASGIClient) -> None:
...
def test_static_config_moderate_cache(static_cache_client: SyncASGIClient) -> None:
...
def test_static_user_uploads_short_cache(static_cache_client: SyncASGIClient) -> None:
...
def test_api_paths_not_modified(static_cache_client: SyncASGIClient) -> None:
...
@pytest.fixture
def versioning_client() -> SyncASGIClient:
    app = FastAPI()
...
def test_versioning_injects_version_in_js(versioning_client: SyncASGIClient) -> None:
    r = versioning_client.get("/page")
    assert r.status_code == 200
    assert f'src="/js/main.js?v={_VERSION}"' in r.text


def test_versioning_injects_version_in_css(versioning_client: SyncASGIClient) -> None:
    r = versioning_client.get("/page")
    assert f'href="/css/style.css?v={_VERSION}"' in r.text


def test_versioning_replaces_existing_query(versioning_client: SyncASGIClient) -> None:
    r = versioning_client.get("/page-existing-query")
    assert f'?v={_VERSION}"' in r.text
    assert "?v=old" not in r.text


def test_versioning_skips_external_urls(versioning_client: SyncASGIClient) -> None:
...
def test_versioning_skips_non_html_responses(versioning_client: SyncASGIClient) -> None:
