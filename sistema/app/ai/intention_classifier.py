...
class IntentionClassifier:
    ...
    LISTAR_ORCAMENTOS_KEYWORDS = [
        r'\b(quais|listar?|liste|lista|ver|mostrar|mostre)\s+(?:s[ãa]o\s+)?(mais\s+)?(?:os\s+)?(?:meus\s+)?or[çc]amentos?\b',
        ...
        r'\blista\s+de\s+or[çc]amentos?\b',
    ]

    LISTAR_CLIENTES_KEYWORDS = [
        r'\b(quais|listar?|liste|lista|ver|mostrar|mostre)\s+(?:s[ãa]o\s+)?(mais\s+)?(?:os\s+)?(?:meus\s+)?clientes?\b',
        r'\b(ultimos?|[úu]ltimos?|recentes?)\s+clientes?\b',
        r'\btodos?\s+(os\s+)?clientes?\b',
        r'\blista\s+completa\s+de\s+clientes?\b',
        r'\blista\s+de\s+clientes?\b',
    ]
    ...
    OPERADOR_KEYWORDS = [
        ...
        r'^[A-Za-z]+-\d+$',
        r'^\s*aprovar?\s+(?:(?:o\s+)?or[çc]amento\s+)?(?:[A-Za-z]+-)?\d+\s*$',
        r'^\s*recusar?\s+(?:(?:o\s+)?or[çc]amento\s+)?(?:[A-Za-z]+-)?\d+\s*$',
    ...
    AJUDA_SISTEMA_KEYWORDS = [
        ...
        r'como\s+(?:eu\s+)?(?:crio|cadastro|faco|faço)\s+(?:um\s+)?(?:cliente|orcamento|servico)',
    ]
