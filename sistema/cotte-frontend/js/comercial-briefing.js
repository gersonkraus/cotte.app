...
(function () {
  ...
  async function _carregarTemplates(tipo, canal) {
    ...
    try {
      ...
      var res = await window.ApiService.get('/comercial/templates' + params);
      _templates = res || [];
      return _templates;
    } catch (e) {
  ...
  async function _fetchBriefing(forcar) {
    ...
    var res = await window.ApiService.get('/comercial/leads/briefing');
  ...
  window.BriefingIA = {
    ...
    enviar: async function (leadId, tipoAcao) {
      ...
      try {
        var endpoint = tipoAcao === 'mensagem_email'
          ? '/comercial/leads/' + leadId + '/email'
          : '/comercial/leads/' + leadId + '/whatsapp';
    ...
    confirmarEtapa: async function (leadId, etapaSugerida) {
      ...
      try {
        await window.ApiService.put('/comercial/leads/' + leadId, { status_pipeline: etapaSugerida });
        _estado[leadId] = 'concluido';
        _salvarEstado();
        _renderBriefing();
      } catch (e) {
    ...
    usarTemplate: async function(leadId, templateId, canal) {
      ...
      try {
        var preview = await window.ApiService.post('/comercial/templates/' + templateId + '/preview?lead_id=' + leadId, {});
