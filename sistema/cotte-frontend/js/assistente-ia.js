...
// ═══════════════════════════════════════════════════════════
// FUNÇÕES UTILITÁRIAS (necessárias para o assistente)
// ═══════════════════════════════════════════════════════════
...
const SUGGESTION_ICONS = {
    ...
    'análise': '🔬', 'analise': '🔬', 'inadimpl': '⚠️', 'desconto': '🏷️',
    'editar': '✏️', 'item': '📌', 'default': '💡'
};
...
function getOrcamentoFollowupSuggestions(data, parsedSuggestions, tipo) {
    ...
    const seq = numero.includes('-') ? numero.split('-')[1] : numero;
    const label = numero || 'este orçamento';
    const extra = tipo === 'orcamento_atualizado'
        ? [
            `Ver ${label}`,
            `Enviar ${seq || label} por WhatsApp`,
            `Aprovar ${label}`,
            `Duplicar ${label}`,
        ]
        : [
            `Ver detalhes do ${label}`,
            `Enviar ${seq || label} por WhatsApp`,
            `Duplicar ${label}`,
            `Simular desconto de 5% no ${label}`,
        ];
    const merged = [...base, ...extra].map(s => String(s || '').trim()).filter(Boolean);
    return Array.from(new Set(merged)).slice(0, 6);
}

// ═══════════════════════════════════════════════════════════
// NOVAS VARIÁVEIS GLOBAIS PARA MELHORIAS
// ═══════════════════════════════════════════════════════════

/**
 * Nova versão de addMessage com virtualização e latency indicator
 */
function addMessage(content, isUser = false, isError = false, isLoadingState = false) {
    const messagesContainer = document.getElementById('chatMessages');
    if (!messagesContainer) return null;
...
function getHumanErrorMessage(error, statusCode) {
    const errorMap = {
        401: {
            ...
            message: 'Sua sessão de login expirou. Faça login novamente para continuar.',
            suggestion: 'Você será redirecionado para a tela de login em 3 segundos.',
            showRetry: false, customClass: 'error-session-expired'
        },
        403: {
            icon: '🚫', title: 'Acesso negado',
            message: 'Você não tem permissão para realizar esta ação.',
            suggestion: 'Entre em contato com o administrador do sistema para solicitar acesso.',
            showRetry: false
        },
        404: {
            icon: '🔍', title: 'Recurso não encontrado',
            message: 'O recurso solicitado não foi encontrado.',
            suggestion: 'Verifique se a URL está correta ou tente uma ação diferente.',
            showRetry: false
        },
        429: {
            icon: '⏱️', title: 'Muitas solicitações',
            message: 'Você está fazendo muitas solicitações simultâneas.',
            suggestion: 'Aguarde alguns segundos antes de tentar novamente.',
            showRetry: true, retryDelay: 5000, customClass: 'error-rate-limit'
        },
        500: {
            icon: '⚙️', title: 'Erro interno do servidor',
            message: 'Ocorreu um problema técnico em nossos servidores.',
            suggestion: 'Tente novamente em alguns instantes. Se o problema persistir, entre em contato com o suporte.',
            showRetry: true, customClass: 'error-network'
        },
        502: {
            icon: '🔌', title: 'Problema de conexão',
            message: 'Não foi possível conectar ao servidor.',
            suggestion: 'Verifique sua conexão com a internet e tente novamente.',
            showRetry: true, customClass: 'error-network'
        },
        503: {
            icon: '🔧', title: 'Serviço temporariamente indisponível',
            message: 'O serviço está em manutenção ou sobrecarregado.',
            suggestion: 'Tente novamente em alguns minutos.',
            showRetry: true, retryDelay: 10000
        },
        504: {
            icon: '⏰', title: 'Tempo limite excedido',
            message: 'O servidor demorou muito para responder.',
            suggestion: 'Tente novamente ou simplifique sua solicitação.',
            showRetry: true, customClass: 'error-network'
        }
    ...
    if (!statusCode && (error.message?.includes('Failed to fetch') || error.message?.includes('conexão') || error.message?.includes('internet'))) {
        return {
            icon: '🌐', title: 'Sem conexão com a internet',
            message: 'Verifique sua conexão com a internet.',
            suggestion: 'Conecte-se a uma rede Wi-Fi ou dados móveis e tente novamente.',
            showRetry: true, customClass: 'error-network'
        };
...
function createErrorCard(errorData, originalError, retryCallback) {
    ...
    if (errorData.suggestion) {
        html += `<div class="error-card-suggestion">
            <div class="error-card-suggestion-title">💡 Sugestão</div>
            ${escapeHtml(errorData.suggestion)}
        </div>`;
    }
    ...
    if (errorData.showRetry && retryCallback) {
        html += `<button type="button" class="error-retry-btn" id="${retryId}" data-tooltip="Tentar novamente">
            🔄 Tentar Novamente
        </button>`;
    }
    
    if (originalError && originalError.message) {
        html += `<span class="error-details-toggle" data-target="${detailsId}">Ver detalhes técnicos</span>
            <div class="error-technical-details" id="${detailsId}">${escapeHtml(originalError.message)}</div>`;
    }
