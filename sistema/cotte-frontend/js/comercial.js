...
async function _carregarEtapasPipelineCampanha() {
  try {
    var stages = await api.get('/comercial/pipeline-stages');
...
async function _carregarImportacoesCampanha() {
  try {
    var lista = await api.get('/comercial/import/list');
...
async function _aplicarFiltrosCampanha() {
  ...
  try {
    ...
    if (importacaoId) {
      var res = await api.get('/comercial/import/' + importacaoId + '/leads');
      ...
      var res2 = await api.get('/comercial/leads?' + params);
...
// Filtros do modal de campanha (Audience Builder)
(function() {
