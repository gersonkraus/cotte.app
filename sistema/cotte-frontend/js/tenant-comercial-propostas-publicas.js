...
async function carregarPropostasPublicas() {
    try {
        const response = await api.get('/tenant/comercial/propostas-publicas');
...
async function salvarPropostaPublica() {
    try {
        ...
        if (id) {
            // Editar
            await api.put(`/tenant/comercial/propostas-publicas/${id}`, dados);
            showToast('Proposta atualizada com sucesso', 'success');
        } else {
            // Criar
            await api.post('/tenant/comercial/propostas-publicas', dados);
