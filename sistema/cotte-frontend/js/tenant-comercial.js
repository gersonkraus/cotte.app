...
async function previewImport() {
  ...
  try {
    if (csvFile) {
      ...
      var response = await api.post('/tenant/comercial/leads/analisar-importacao', { texto: text });
...
async function executeImport() {
  ...
  await withBtnLoading(btn, async function() {
    try {
      ...
      var result = await api.post('/tenant/comercial/leads/importar', {
...
async function excluirLeadLote(leadId) {
  ...
  try {
    await api.delete('/tenant/comercial/leads/' + leadId);
...
async function excluirTodosLote() {
  ...
  try {
    ...
    await Promise.all(ids.map(function(id) { return api.delete('/tenant/comercial/leads/' + id); }));
...
async function enviarWhatsAppLote() {
  ...
  try {
    var result = await api.post('/tenant/comercial/leads/enviar-lote', {
...
async function carregarSegmentosImportacao() {
  try {
    var segmentos = await api.get('/tenant/comercial/segmentos?ativo=true');
...
async function carregarTemplatesImportacao() {
  try {
    var templates = await api.get('/tenant/comercial/templates?ativo=true');
...
async function carregarHistoricoImportacoes() {
  ...
  try {
    var importacoes = await api.get('/tenant/comercial/import/list');
...
async function abrirImportacaoLeads(importacaoId, nome) {
  try {
    var data = await api.get('/tenant/comercial/import/' + importacaoId + '/leads');
...
async function excluirImportacao(id, nome) {
  ...
  try {
    await api.delete('/tenant/comercial/import/' + id);
...
async function carregarDashboard() {
  try {
    var results = await Promise.all([
      api.get('/tenant/comercial/dashboard'),
      api.get('/tenant/comercial/leads/follow-ups-hoje'),
      api.get('/tenant/comercial/leads/recentes?limit=5'),
    ]);
...
async function carregarNovosClientes() {
  ...
  try {
    var origens = await api.get('/tenant/comercial/origens?ativo=true');
    ...
    var data = await api.get('/tenant/comercial/leads?status=novo&origem_lead_id=' + origemLp.id + '&per_page=10');
...
async function abrirDetalhe(id) {
  ...
  try {
    ...
    try {
      propostasLead = await api.get('/tenant/comercial/propostas-publicas/leads/' + id + '/propostas');
...
async function garantirPropostasPublicasCache() {
  ...
  try {
    propostasPublicasCache = await api.get('/tenant/comercial/propostas-publicas');
...
async function aplicarTemplate(prefix) {
  ...
  try {
    var preview = await api.post('/tenant/comercial/templates/' + tplId + '/preview', { lead_id: leadAtualId });
...
async function enviarWhatsApp() {
  ...
  await withBtnLoading(btn, async function() {
    try {
      await api.post('/tenant/comercial/leads/' + leadAtualId + '/whatsapp', { mensagem: msg });
...
async function enviarEmail() {
  ...
  await withBtnLoading(btn, async function() {
    try {
      await api.post('/tenant/comercial/leads/' + leadAtualId + '/email', { assunto: assunto, mensagem: msg });
...
async function confirmarReenvioProposta() {
  ...
  try {
    var resp = await api.post('/tenant/comercial/propostas-publicas/leads/' + pend.leadId + '/propostas?force=true', {
    ...
    if (canal === 'whatsapp') {
      ...
      var textoWa = mensagemWhatsAppComLinkProposta(resp.slug, nomeProposta);
      try {
        await api.post('/tenant/comercial/leads/' + pend.leadId + '/whatsapp', { mensagem: textoWa });
      ...
      var corpoRe =
        ...
        linkRe +
        '\n\nQualquer dúvida, estamos à disposição.';
      await api.post('/tenant/comercial/leads/' + pend.leadId + '/email', {
        assunto: assuntoRe,
        mensagem: corpoRe
      });
...
async function carregarLembretes() {
  ...
  var url = '/tenant/comercial/lembretes';
...
function abrirModalLembrete(preLeadId) {
  ...
  document.getElementById('lemb-lead-dropdown').style.display = 'none';
  if (preLeadId) {
    api.get('/tenant/comercial/leads/' + preLeadId).then(function(l) {
...
async function carregarSegmentos() {
  try {
    var items = await api.get('/tenant/comercial/segmentos');
...
async function salvarSegmento() {
  ...
  try {
    if (id) { await api.patch('/tenant/comercial/segmentos/' + id, { nome: nome }); showToast('Segmento atualizado!', 'success'); }
    else { await api.post('/tenant/comercial/segmentos', { nome: nome }); showToast('Segmento criado!', 'success'); }
    fecharModal('modal-segmento'); carregarSegmentos(); await carregarCadastrosCache();
  } catch(e) { showToast(e.message || 'Erro', 'error'); }
}

async function toggleSegmento(id, ativo) {
  try { await api.patch('/tenant/comercial/segmentos/' + id, { ativo: ativo }); carregarSegmentos(); await carregarCadastrosCache(); }
  catch(e) { showToast('Erro', 'error'); }
}

async function carregarOrigens() {
  try {
    var items = await api.get('/tenant/comercial/origens');
...
async function salvarOrigem() {
  ...
  try {
    if (id) { await api.patch('/tenant/comercial/origens/' + id, { nome: nome }); showToast('Origem atualizada!', 'success'); }
    else { await api.post('/tenant/comercial/origens', { nome: nome }); showToast('Origem criada!', 'success'); }
    fecharModal('modal-origem'); carregarOrigens(); await carregarCadastrosCache();
  } catch(e) { showToast(e.message || 'Erro', 'error'); }
}

async function toggleOrigem(id, ativo) {
  try { await api.patch('/tenant/comercial/origens/' + id, { ativo: ativo }); carregarOrigens(); await carregarCadastrosCache(); }
  catch(e) { showToast('Erro', 'error'); }
}
...
async function carregarPipelineStagesUI() {
  try {
    var items = await api.get('/tenant/comercial/pipeline-stages');
...
async function salvarPipelineStage() {
  ...
  try {
    if (id) {
      await api.patch('/tenant/comercial/pipeline-stages/' + id, payload);
      showToast('Etapa atualizada!', 'success');
    } else {
      payload.slug = slug;
      await api.post('/tenant/comercial/pipeline-stages', payload);
...
async function togglePipelineStage(id, ativo) {
  try {
    await api.patch('/tenant/comercial/pipeline-stages/' + id, { ativo: ativo });
    await carregarPipelineStagesUI();
  } catch(e) { showToast('Erro', 'error'); }
}
...
async function excluirPipelineStage(id, label) {
  ...
  try {
    await api.delete('/tenant/comercial/pipeline-stages/' + id);
...
async function carregarConfig() {
  try {
    var c = await api.get('/tenant/comercial/config');
...
async function salvarConfig() {
  ...
  try { await api.patch('/tenant/comercial/config', data); showToast('Configurações salvas!', 'success'); }
...
if (btnNovoLead) {
  btnNovoLead.addEventListener('click', function(e) {
    e.preventDefault();
    try {
      abrirModalLead();
    } catch (err) {
      console.error('[tenant-comercial] Falha ao abrir modal de lead:', err);
      if (typeof showToast === 'function') {
        showToast('Erro ao abrir formulário de lead', 'error');
      }
    }
  });
}
...
// Evita dupla submissão: o botão "Salvar" do modal-template já chama
// TemplatesManager.salvarTemplate() via onclick no HTML.
